    ...
    const handleInsertRowAt = (records, row, isInsertNextRow) => {
      ...
      if (_xeUtils.default.eqNull(row)) {
        ...
        if (row === -1) {
          ...
          // 如果为虚拟树
          if (treeConfig && transform) {
            const matchMapObj = _xeUtils.default.findTree(tableFullTreeData, item => row[rowField] === item[rowField], {
              children: mapChildrenField
            });
            if (matchMapObj) {
              const {
              ...
              // 源
              if (parentRow) {
                const matchObj = _xeUtils.default.findTree(tableFullTreeData, item => row[rowField] === item[rowField], {
